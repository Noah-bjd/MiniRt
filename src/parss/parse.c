...
/*   Updated: 2023/06/11 05:28:34 by faksouss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	len_elem(char **string, int *pl, int *sp, int *cy)
{
	...
	while (string[i])
	{
		...
		if (!ft_strncmp(ptr, "pl ", strlen("pl ")))
			...
			(*cy)++;
		else if ((ft_strncmp(string[i], "A ", 2) && ft_strncmp(string[i], "C ",
					2) && ft_strncmp(string[i], "L ", 2))
			&& (space(string[i]) != (int)ft_strlen(string[i])))
		{
			printf("-> %s\n", string[i]);
